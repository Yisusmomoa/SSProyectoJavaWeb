...
DELIMITER ;
    ...
    public static Materia getMateriaByID(int claveMateriaGrupo) throws SQLException{
        Connection con = null;
        try {
            con=DbConnection.getConnection();
...
DELIMITER ;
    ...
    public static List<Materia> getBusquedaMateria(String inputBusqueda) throws SQLException{
        List<Materia> listaMaterias=new ArrayList<>();
        Connection con = null;
        try {
            con=DbConnection.getConnection();
            String sql="Call getBusquedaMateria(?)";
            CallableStatement statement=con.prepareCall(sql);
    ...
    USE `proyectojavawebss`;
    DROP procedure IF EXISTS `proyectojavawebss`.`eliminarMateria`;
    ;
    ...
    public static int eliminarMateria(int claveMateria) throws SQLException{
        Connection con = null;
        try {
            con=DbConnection.getConnection();
            String sql="Call eliminarMateria(?)";
            CallableStatement statement=con.prepareCall(sql);
            statement.setInt(1, claveMateria);
            return statement.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(MateriaDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    /**
     USE `proyectojavawebss`;
DROP procedure IF EXISTS `editarMateria`;

USE `proyectojavawebss`;
DROP procedure IF EXISTS `proyectojavawebss`.`editarMateria`;
;

DELIMITER $$
USE `proyectojavawebss`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `editarMateria`(
IN `pclaveMateria` int,
IN `pnombreMateria` varchar(255),
IN `pestatus` boolean
)
...
;
    ...
    public static int editarMateria (Materia materia) throws SQLException{
         Connection con = null;
        try {
            con=DbConnection.getConnection();
            String sql="Call editarMateria(?,?,?)";
            CallableStatement statement=con.prepareCall(sql);
            statement.setInt(1, materia.getClaveMateria());
            statement.setString(2, materia.getNombreMateria());
            statement.setBoolean(3, materia.isEstatus());
            return statement.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(MateriaDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
