 ...
 * @author bjls2
 */
@WebServlet(name = "FiltrarIdGrupo", urlPatterns = {"/FiltrarIdGrupo"})
public class FiltrarIdGrupo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet FiltrarIdGrupo</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet FiltrarIdGrupo at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String FiltrarGrupos=request.getParameter("search");
        List<Grupo> listaGrupos=new ArrayList<>();
        try {
            listaGrupos=grupoDAO.getGruposByMateria(FiltrarGrupos);
            json.put("status", 200);
            String Tabla;
            Tabla="";
            for(Grupo grupoAux:listaGrupos){
                ...
                Tabla=Tabla.concat("<td class='text-center align-middle'>");
                    Tabla=Tabla.concat("<button type='button'\n" +
"                        value='Inscribir'\n" +
"                        id='InscribirMateria' \n" +
"                        class='btn btn-secondary'>Inscribir</button>");
            ...
            Logger.getLogger(FiltrarIdGrupo.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
